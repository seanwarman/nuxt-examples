...
THREE.OutlineEffect = function(renderer, parameters) {
  ...
  var defaultThickness =
    parameters.defaultThickness !== undefined
      ...
      : 0.006;
  var defaultColor = new THREE.Color().fromArray(
  ...
  function createMaterial(originalMaterial) {
    ...
    if (shaderID !== undefined) {
      ...
      if (
        ...
        console.warn(
          'THREE.OutlineEffect requires both vec3 position and normal attributes in vertex shader, ' +
            'does not draw outline for ' +
  ...
  function setOutlineMaterial(object) {
    if (object.material === undefined || object.type === 'Sprite') return;
    if (contains(object.name, exludedMeshes)) {
      if (object.name.startsWith('anno') && object.material.map != null) {
  ...
  function restoreOriginalMaterial(object) {
    if (object.material === undefined || object.type === 'Sprite') return;
    if (contains(object.name, exludedMeshes)) return;
    if (Array.isArray(object.material)) {
      for (var i = 0, il = object.material.length; i < il; i++) {
